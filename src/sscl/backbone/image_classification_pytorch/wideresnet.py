...
from sscl.backbone import BaseBackbone
...
class WideResNet(BaseBackbone):
    def __init__(self, num_classes=10, block=WideBasic, depth=40, widen_factor=10):
        super().__init__()
...
def wideresnet_40_10(num_classes=10):
    net = WideResNet(num_classes=num_classes, block=WideBasic, depth=40, widen_factor=10)
    return net
...
def save_model_to_onnx():
    ...
    net = wideresnet_40_10().cuda()
    onnx_path = '/data/gxy/legodnn-public-version_9.27/cv_task/image_classification/cifar/onnx_model_weight/cifar10_wideresnet_40_10.onnx'
    torch.onnx.export(net, data, onnx_path)
    onnx.save(onnx.shape_inference.infer_shapes(onnx.load(onnx_path)), onnx_path)

    net = wideresnet_40_10(num_classes=100).cuda()
    onnx_path = '/data/gxy/legodnn-public-version_9.27/cv_task/image_classification/cifar/onnx_model_weight/cifar100_wideresnet_40_10.onnx'
    torch.onnx.export(net, data, onnx_path)
    onnx.save(onnx.shape_inference.infer_shapes(onnx.load(onnx_path)), onnx_path)
