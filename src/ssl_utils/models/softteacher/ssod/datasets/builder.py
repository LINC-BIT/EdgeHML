...
"""
Collate 函数是 Dataloader 中的聚合函数，目的是将 batch 个数据聚合
为 batch 的 4d 格式数据。
因为 SoftTeacher 中两条 pipeline，所以需要重写 collate 数据聚合函数，
也因此需要重写 build_sampler 和 build_dataloader
"""
...
def build_dataloader(
    ...
    **kwargs,
    ...
    if dist:
        ...
        sampler = (
            build_sampler(sampler_cfg, group=False, default_args=default_sampler_cfg)
            if shuffle
            else None
        )
